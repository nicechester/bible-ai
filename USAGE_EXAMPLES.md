...
```
"백부장이 나온 구절 찾아줘"
→ KEYWORD 검색 (정확한 단어 매칭)

"사랑의 의미를 알려줘"
→ SEMANTIC 검색 (의미 기반 검색)
...
"믿음이 몇 번 나와?"
→ STATISTICS (통계 - LLM 없이 직접 응답)

"사랑에 대한 구절 찾아줘"
→ LIST (구절 목록 - LLM 없이 직접 응답)
```
...
**처리 과정:**
```
1단계: Bi-encoder로 50개 후보 검색 (빠름)
    └── 임베딩 유사도 기반 검색
    
2단계: 재순위 및 필터링
    └── 키워드 부스트 (정확한 단어 매칭 시 가산점)
    └── 맥락 필터 (지정된 책/성경 구분으로 제한)
    └── 최종 10개 결과 선별
```
